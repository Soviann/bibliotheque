...
describe("Trash", () => {
  ...
  it("has restore buttons for each trashed comic", async () => {
    const comics = [createMockComicSeries({ id: 1, title: "Trashed" })];
  ...
  it("shows confirm modal when permanent delete is clicked", async () => {
    ...
    const comics = [createMockComicSeries({ id: 1, title: "To Delete" })];
    ...
    expect(
      screen.getByText(
        "Cette action est irréversible. La série sera définitivement supprimée.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("fires DELETE API call when confirming permanent delete", async () => {
    ...
    const comics = [createMockComicSeries({ id: 1, title: "Perm Delete" })];
  ...
  it("triggers restore when restore button is clicked", async () => {
    const user = userEvent.setup();
    const comics = [createMockComicSeries({ id: 1, title: "Restore Me" })];
  ...
  it("shows success toast after restore", async () => {
    const user = userEvent.setup();
    const comics = [createMockComicSeries({ id: 1, title: "Restored Comic" })];
  ...
  it("shows success toast after permanent delete", async () => {
    const user = userEvent.setup();
    const comics = [createMockComicSeries({ id: 1, title: "Gone Forever" })];
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith(
        "Gone Forever supprimée définitivement",
      );
    });
  });
  ...
  it("renders coverImage with priority over coverUrl (local first)", async () => {
    ...
    await waitFor(() => {
      ...
      expect(img.getAttribute("src")).toContain(
        "/media/cache/cover_thumbnail/uploads/covers/local.webp",
      );
    });
  });
